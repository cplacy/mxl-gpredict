        ...
        gtk_rot_ctrl_update   (GtkRotCtrl *ctrl, gdouble t)
{
    ...
    // Get target - first sat in range
    current = ctrl->target;
    n = g_slist_length (ctrl->sats);
    // Check for satellites currently in view
    sat_index = -1;
    for (i=0; i<n; ++i)
    {
        temp = SAT (g_slist_nth_data (ctrl->sats, i));
        // A satellite's in view! track it!
        if(temp->el > 0.0)
        {
            sat_index = i;
            break;
    ...
    // If nothing in view, check next AOS
    if(sat_index == -1)
    {
        for (i=0; i<n; ++i)
        {
            temp = SAT (g_slist_nth_data (ctrl->sats, i));
            pass_temp = get_pass (temp, ctrl->qth, ctrl->t, 3.0);
            if(i==0)
            {
                aos_time = pass_temp->aos;
                sat_index = 0; 
            } else if(pass_temp->aos < aos_time) {
                aos_time = pass_temp->aos;
                sat_index = i; 
            }
            free_pass(pass_temp);
    ...
    // Now select the sat we determined
    temp = SAT (g_slist_nth_data (ctrl->sats, sat_index));
    if(temp != current)
    {
        // Change targets!
        /* update next pass */
        ...
        buff = g_strdup_printf ("%s", temp->nickname);
        gtk_label_set_text (GTK_LABEL (ctrl->SatName), buff);
        g_free(buff);
...
{
    GtkWidget *frame,*table,*label,*satsel,*track,*satlabel;
    gchar *buff;
    guint i, n;
    sat_t *sat = NULL;
    ...
    if(n>0)
    {
        ...
        if (sat) {
            satlabel = gtk_label_new("Charlie's Multi-track");
        }
        else
        {
            satlabel = gtk_label_new("No Satellites!");
        }
    }
    ctrl->SatName = satlabel;
    // g_signal_connect (satsel, "changed", G_CALLBACK (sat_selected_cb), ctrl);
    gtk_table_attach_defaults (GTK_TABLE (table), satlabel, 0, 2, 0, 1);
